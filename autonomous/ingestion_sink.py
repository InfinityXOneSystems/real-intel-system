...
def write_records_to_gcs(records: List[Dict], bucket: str, path: str, dry_run: bool = True) -> dict:
    ...
    try:
        from services.real_estate_intelligence.autonomous.storage_helper import write_to_gcs_or_local
        ok, uri = write_to_gcs_or_local(bucket, path, json.dumps(records).encode('utf-8'))
        if ok:
            res = {"success": True, "uri": uri, "dry_run": False}
            # Optionally insert into BigQuery if configured via env
            try:
                import os
                if os.getenv('AUTONOMY_INSERT_BIGQUERY', 'false').lower() in ('1','true'):
                    from services.real_estate_intelligence.autonomous.bigquery_helper import insert_rows
