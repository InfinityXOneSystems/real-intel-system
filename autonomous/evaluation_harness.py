...
def run_scoring_pipeline(records: List[Dict], dry_run: bool = True) -> dict:
    ...
    try:
        ...
        jr = resp.json()
        return {"success": True, "scored_count": len(jr.get('results', [])), "results": jr.get('results'), "dry_run": False}
    except Exception:
        logger.exception('Predict endpoint unavailable; using production_scorer')
        try:
            from services.real_estate_intelligence.autonomous.production_scorer import score_records
            results = []
            ...
            for r, s in zip(records, scores):
                ...
                rr.update(s)
                results.append(rr)
            return {"success": True, "scored_count": len(results), "results": results, "dry_run": False}
        except Exception:
            ...
            for r in records:
                ...
                rr['score'] = score
                out.append(rr)
            return {"success": True, "scored_count": len(out), "results": out, "dry_run": False}
