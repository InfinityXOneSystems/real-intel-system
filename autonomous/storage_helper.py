...
def write_to_gcs_or_local(bucket: str, object_name: str, data_bytes: bytes, retries: int = 3, backoff: float = 1.0) -> Tuple[bool, str]:
    ...
    try:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                logger.warning('GCS upload attempt %d failed: %s; retrying in %.1fs', attempt, e, wait)
                time.sleep(wait)
