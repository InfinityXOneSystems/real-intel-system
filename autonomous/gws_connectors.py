...
def send_gmail_message(
    to: str, subject: str, body: str, creds_path: Optional[str] = None
):
    creds = _get_creds(creds_path)
    ...
    return {
        ...
        "note": "Use send_gmail_raw to send base64-encoded message",
    }


def create_calendar_event(
    calendar_id: str, event: Dict[str, Any], creds_path: Optional[str] = None
):
    creds = _get_creds(creds_path)
...
def upload_file_to_drive(
    folder_id: str, name: str, file_path: str, creds_path: Optional[str] = None
):
    creds = _get_creds(creds_path)
