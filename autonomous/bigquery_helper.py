...
def insert_rows(project: str, dataset: str, table: str, rows: List[Dict], retries: int = 5, backoff: float = 1.0, max_backoff: float = 16.0) -> Dict:
    ...
    while True:
        try:
            ...
            logger.warning('BigQuery insert transient error, retry %d/%d in %.1fs: %s', attempt, retries, sleep_for, str(e))
