...
def read_sheet(sheet_id: str, range_name: str, creds) -> List[Dict]:
    ...
    for r in rows:
        entry = {
            header[i].strip(): (r[i].strip() if i < len(r) else "")
...
def build_job(row: Dict) -> Dict:
    job = {
        ...
        "meta": {
            ...
            if k not in ("source", "type", "priority", "seed_url", "seed", "county")
        },
    }
    return job
...
def main():
    ...
    p.add_argument(
        "--endpoint",
        default="https://crawler-control-896380409704.us-east1.run.app/crawl",
    )
    ...
    if not args.creds or not os.path.exists(args.creds):
        print(
            "ERROR: Google credentials JSON not found. Set GOOGLE_APPLICATION_CREDENTIALS or pass --creds",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    try:
        ...
        try:
            ...
            print(
                "Warning: service account file invalid; attempting Application Default Credentials (ADC) fallback"
            )
